...
This repo has submodules (`ondewo-proto-compiler`, `src/ondewo-csi-api`). If a submodule's working
...
```text
Release ONDEWO CSI Nodejs Client ${ONDEWO_CSI_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO CSI Nodejs Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO CSI Nodejs Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/csi-client-nodejs`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/csi-client-nodejs versions --json
```
