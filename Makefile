...
GOOGLE_PROTOS_DIR=${CSI_APIS_DIR}/google
GITHUB_GH_TOKEN?=
NPM_AUTOMATION_TOKEN?=
IMAGE_UTILS_NAME=ondewo-csi-client-utils-nodejs:${ONDEWO_CSI_VERSION}
PRETTIER_WRITE?=

CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO CSI Nodejs Client ${ONDEWO_CSI_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-csi-client-nodejs"
...
# only in the root copy is destroyed on the next build. src/README.md is the source of truth
# (covered by `git add src`); this stages the generated copy so the tracked file cannot drift.
	git add README.md
	git add RELEASE.md
...
build: check_out_correct_submodule_versions build_compiler update_package npm_run_build ## Build Code with Proto-Compiler
	...
	-cd src/ondewo-csi-api && git checkout -- '**/*.proto' && cd ../..
	-cd src/ondewo-csi-api && git checkout -- google && cd ../..
	cp src/README.md .
	cp src/RELEASE.md .
